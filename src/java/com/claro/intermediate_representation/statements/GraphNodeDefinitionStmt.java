    ...
    if (!StructuralConcreteGenericTypeValidationUtil.isDeeplyImmutable(this.actualNodeType)) {
      this.nodeExpr.logTypeError(
          ClaroTypeException.forIllegalUseOfMutableTypeAsGraphNodeResultType(
              this.actualNodeType,
              this.nodeName,
              this.actualNodeType instanceof SupportsMutableVariant<?>
              ? ((SupportsMutableVariant<?>) this.actualNodeType).toDeeplyImmutableVariant()
              : Types.FutureType.wrapping(
                  ((SupportsMutableVariant<?>) actualNodeType.parameterizedTypeArgs()
                      .get(Types.FutureType.PARAMETERIZED_TYPE_KEY))
                      .toDeeplyImmutableVariant())
          ));
    }
  }
